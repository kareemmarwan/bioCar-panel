...
export default function Dashboard() {
  ...
  return (
    <Layout>
      {/* العناوين - جعلناها متجاوبة padding وهامش */}
      <div className="flex flex-col gap-1 mb-6 md:mb-8 pt-10 md:pt-0">
        <h1 className="text-2xl md:text-3xl font-display font-bold text-foreground tracking-tight">Dashboard</h1>
        <p className="text-sm md:text-base text-muted-foreground">Overview of your pharmacy performance and real-time metrics.</p>
      </div>

      {/* الإحصائيات العلوية - Stat Cards */}
      {/* التعديل هنا: grid-cols-1 افتراضياً للموبايل */}
      ...
      {/* التعديل هنا: شبكة مرنة للموبايل grid-cols-1 وفي الشاشات الكبيرة lg:grid-cols-7 */}
      <div className="grid grid-cols-1 lg:grid-cols-7 gap-6">
        
        {/* الرسم البياني - Sales Chart */}
        <Card className="col-span-1 lg:col-span-4 shadow-sm border-border/60 hover:shadow-md transition-shadow">
          <CardHeader>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={data?.monthlySales || []}>
                    ...
                    <Bar 
                      ...
                      fill="hsl(var(--primary))" 
                      radius={[6, 6, 0, 0]} 
                      barSize={45} // الـ Recharts ستتكفل بتقليص الحجم تلقائياً للموبايل بفضل الـ ResponsiveContainer
                    />
                  </BarChart>
                </ResponsiveContainer>
              )}
          ...
          <CardContent>
            <div className="space-y-4">
              {isLoading ? (
                ...
                (data?.recentOrders || []).map((order) => (
                  <div key={order.id} className="flex items-center justify-between p-3 md:p-4 bg-muted/20 rounded-xl hover:bg-muted/40 transition-all group">
                    <div className="flex flex-col">
                      <span className="font-bold text-sm text-foreground group-hover:text-primary transition-colors">
                        {order.customerName}
                      </span>
